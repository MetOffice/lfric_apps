...
subroutine lw_inc_code(nlayers, n_profile,                                     &
  ...
  integer(i_def) :: max_threads, soc_lw_inc_block, seg_start, seg_end, &
  ...
  do k=0, nlayers
    ...
    if (n_profile_list > 0) then
      ...
      do ll=1, n_profile_list, soc_lw_inc_block
        seg_start = ll

        if ((seg_start + soc_lw_inc_block) -1 >= n_profile_list) then
          seg_end = n_profile_list
          n_profile_list_seg = (seg_end - seg_start) + 1
        else
          seg_end = (seg_start + soc_lw_inc_block) - 1
          n_profile_list_seg = soc_lw_inc_block
        end if

        ! Calculate the LW increment fluxes
        call runes(n_profile_list_seg, nlayers, lwinc_diag,                    &
          spectrum_name          = 'lwinc',                                    &
