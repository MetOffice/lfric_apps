...
END TYPE atl_project_eos_pressure_kernel_type
  ...
  contains
  subroutine atl_project_eos_pressure_code(cell, nlayers, exner, rho, theta, moist_dyn_gas, ls_rho, ls_theta, ls_moist_dyn_gas, &
...
&undf_chi, map_chi, chi_basis, chi_diff_basis, ndf_pid, undf_pid, map_pid, nqp_h, nqp_v, wqp_h, wqp_v)
    ...
    do k = nlayers - 1, 0, -1
      ...
      enddo
      call coordinate_jacobian(coord_system, geometry, topology, scaled_radius, ndf_chi, nqp_h, nqp_v, chi1_e, chi2_e, chi3_e, &
&ipanel, chi_basis, chi_diff_basis, jac, dj)
      ...
      do df = ndf_w3, 1, -1
        do df2 = ndf_w3, 1, -1
          r_exner(df2) = r_exner(df2) + m3_inv(ik,df,df2) * exner_e(df)
          exner_e(df) = 0.0
        enddo
      enddo
      do qp2 = nqp_v, 1, -1
