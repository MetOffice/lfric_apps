...
!$OMP private(i, k, bc, qs, tmp1, tmp2)                                        &
...
do k = 1, bl_levels
  ...
  if (l_noice_in_turb) then
    ...
    if ( l_mr_physics ) then
      call qsat_wat_mix(qs,t(:,k),p(:,k),tdims%i_end)
    else
      call qsat_wat(qs,t(:,k),p(:,k),tdims%i_end)
    end if
  else
    if ( l_mr_physics ) then
      call qsat_mix(qs,t(:,k),p(:,k),tdims%i_end)
    else
      call qsat(qs,t(:,k),p(:,k),tdims%i_end)
    end if
  ...
  do i = tdims%i_start, tdims%i_end
    if (t(i,k) > tm .or. l_noice_in_turb) then
      tmp1(i) = lc
      tmp2(i) = lcrcp
  ...
  do i = tdims%i_start, tdims%i_end
    ...
    a_qs(i,k) = 1.0_prec / (1.0_prec + tmp2(i)*dqsdt(i,k))

    a_dqsdt(i,k) = a_qs(i,k) * dqsdt(i,k)

    bc = tmp2(i)*bt(i,k) - etar*bq(i,k)
