...
real(kind=r_bl), intent(in out) ::                                             &
          dqw_inv_wtrac(pdims%i_start:pdims%i_end,   &
...
!$OMP  private (i, k, dz_disc, qw_lapse)
  do i = pdims%i_start, pdims%i_end

    dqw_inv_wtrac(i,i_wt) = zero

    if (inv_calc(i)) then   ! do inversion calculation

      k = k_inv_start(i)       ! Use starting value
      !..by default, keep ZH at the half-level where it was diagnosed
      !..initially and use grid-level jumps

      dqw_inv_wtrac(i,i_wt) = wtrac_bl(i_wt)%qw(i,k+1)                     &
                                 - wtrac_bl(i_wt)%qw(i,k)

      if (disc_inv(i) == 1) then    ! Discontinuous inversion
        ...
        if (wtrac_info(i_wt)%wt_class == noniso_class_wtrac) then
          ...
          if ( k  <=  bl_levels-3 ) then
            if (qw_lapse_zero(i)) then
              ...
              qw_lapse =                                                       &
               ( wtrac_bl(i_wt)%qw(i,k+3) - wtrac_bl(i_wt)%qw(i,k+2) )     &
                 *rdz(i,k+3)
            end if
          end if
          ...
          if (dqw_meth(i) > 0) then

              dqw_inv_wtrac(i,i_wt) = wtrac_bl(i_wt)%qw(i,k+2)             &
                                        - wtrac_bl(i_wt)%qw(i,k)

            if (dqw_meth(i) == 2) then
                dqw_inv_wtrac(i,i_wt) = dqw_inv_wtrac(i,i_wt) -            &
                         one_half*dqw_inv_wtrac(i,i_wt)
            else if (dqw_meth(i) == 3) then
                dqw_inv_wtrac(i,i_wt) = dqw_inv_wtrac(i,i_wt) -            &
                         qw_lapse*dz_disc
            end if
          end if
          ...
          if (wtrac_bl(i_wt)%qw(i,k+2) < wtrac_bl(i_wt)%qw(i,k+1) .and.    &
              wtrac_bl(i_wt)%qw(i,k+1) < wtrac_bl(i_wt)%qw(i,k) ) then
              ...
              dqw_inv_wtrac(i,i_wt) = wtrac_bl(i_wt)%qw(i,k+2)             &
                                      - wtrac_bl(i_wt)%qw(i,k)
            if ( dqw_inv_wtrac(i,i_wt) < zero ) then
                dqw_inv_wtrac(i,i_wt) = dqw_inv_wtrac(i,i_wt) -            &
                       max( one_half*dqw_inv_wtrac(i,i_wt), qw_lapse*dz_disc )
            end if
          else if (                                                            &
              wtrac_bl(i_wt)%qw(i,k+2) > wtrac_bl(i_wt)%qw(i,k+1) .and.    &
              wtrac_bl(i_wt)%qw(i,k+1) > wtrac_bl(i_wt)%qw(i,k) ) then
...
real(kind=r_bl), intent(in) :: dzl(tdims%i_start:tdims%i_end,bl_levels)
...
real(kind=r_bl), intent(in) :: dqw_inv_wtrac                                   &
               (pdims%i_start:pdims%i_end,n_wtrac)
...
integer :: i,i_wt,k        ! Loop counters
...
logical :: moisten_wt(pdims%i_start:pdims%i_end)
                                ! Indicator of whether inversion grid-level
                                !   should moisten this timestep (or dry)
...
do i_wt = 1, n_wtrac
!$OMP  PARALLEL do SCHEDULE(STATIC) DEFAULT(SHARED)                            &
!$OMP  private (i, k, ml_tend, fa_tend, inv_tend, totqf_efl)
  do i = pdims%i_start, pdims%i_end
    ...
    if ( t_frac(i)  >   zero ) then
      ...
      if (inv_type == 'SML') then
          ...
          ! inversion height
          wtrac_bl(i_wt)%totqf_zh(i) =                                       &
                                     - we_rho(i)*dqw_inv_wtrac(i,i_wt)     &
                                     + fq_nt_zh_wtrac(i,i_wt)
          ! Interpolate to entrainment flux-level below
          totqf_efl = wtrac_bl(i_wt)%fq_nt(i,1) + wtrac_bl(i_wt)%fqw(i,1)  &
                      +  zrzi(i) * ( wtrac_bl(i_wt)%totqf_zh(i)            &
                       - wtrac_bl(i_wt)%fq_nt(i,1)                           &
                       - wtrac_bl(i_wt)%fqw(i,1) )

        ml_tend = - (  wtrac_bl(i_wt)%totqf_zh(i)                            &
                      - wtrac_bl(i_wt)%fq_nt(i,1)                            &
                      - wtrac_bl(i_wt)%fqw(i,1) ) /zh(i)

      else if (inv_type == 'DSC') then
          ! Calculate total (turb+micro) QW flux at subgrid inversion
          wtrac_bl(i_wt)%totqf_zhsc(i) =                                     &
                            - we_rho(i)*dqw_inv_wtrac(i,i_wt)              &
                               + fq_nt_zh_wtrac(i,i_wt)
          ! Interpolate to entrainment flux-level
          totqf_efl = wtrac_bl(i_wt)%fq_nt_dscb(i) +                         &
              (wtrac_bl(i_wt)%totqf_zhsc(i)-wtrac_bl(i_wt)%fq_nt_dscb(i))  &
                              * zrzi(i)
          ! Note, here zh = dscdepth
        ml_tend = - ( wtrac_bl(i_wt)%totqf_zhsc(i)                           &
                         - wtrac_bl(i_wt)%fq_nt_dscb(i) )/zh(i)
      ...
      fa_tend = zero
      if ( k+1  <=  bl_levels )                                                &
        fa_tend = - ( wtrac_bl(i_wt)%fq_nt(i,k+2)                            &
                           - wtrac_bl(i_wt)%fq_nt(i,k+1) )                   &
                             / dzl(i,k+1)
      ...
      if (wtrac_info(i_wt)%wt_class == noniso_class_wtrac) then
        ...
        if ( moisten(i) ) then
          ...
          if ( totqf_efl_meth1(i) == 1) then
              totqf_efl = wtrac_bl(i_wt)%fq_nt(i,k+1)+inv_tend*dzl(i,k)
            end if
          if (we_parm(i)+w_ls(i)  >=  zero) then
              ...
              ! to TOTQF_EFL:
            if (inv_type == 'SML') then
                inv_tend = (wtrac_bl(i_wt)%qw(i,k-1)                         &
                           - wtrac_bl(i_wt)%qw(i,k))/timestep                &
                           + (wtrac_bl(i_wt)%fq_nt(i,1) +                    &
                              wtrac_bl(i_wt)%fqw(i,1))/z_uv_k(i)
            else if (inv_type == 'DSC') then
                ! Note, here z_uv_k = dscdepth
                inv_tend = (wtrac_bl(i_wt)%qw(i,k-1)                         &
                           - wtrac_bl(i_wt)%qw(i,k))/timestep                &
                           + wtrac_bl(i_wt)%fq_nt_dscb(i)/z_uv_k(i)
            end if
            if ( totqf_efl_meth2(i) == 1) then
                totqf_efl =                                                    &
                        (wtrac_bl(i_wt)%fq_nt(i,k+1)+inv_tend*dzl(i,k))    &
                         /(one+ dzl(i,k)/z_uv_k(i))
            end if
              end if
          else
              if (totqf_efl_meth1(i) == 1) then
      ...
      wtrac_bl(i_wt)%fqw(i,k) = t_frac(i) *                                &
                     ( totqf_efl - wtrac_bl(i_wt)%fq_nt(i,k) )
